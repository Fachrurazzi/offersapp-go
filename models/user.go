...
func (u *User) Register(conn *pgx.Conn) error {
	...
	rows := conn.QueryRow(context.Background(), "SELECT id from user_account WHERE email = $1", u.Email)
	userLookup :=  User{}
	err := rows.Scan(&userLookup)
...
func (u *User) IsAuthenticated(conn *pgx.Conn) error {
	rows := conn.QueryRow(context.Background(), "SELECT id, password_hash from user_account WHERE email = $1", u.Email)
