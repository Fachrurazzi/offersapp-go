...
func (i *Item) Create(conn *pgx.Conn, userID string) error {
	...
	rows := conn.QueryRow(context.Background(), "INSERT INTO item (title, notes, seller_id, price_in_cents, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, seller_id", i.Title, i.Notes, userID, i.PriceInCents, now, now)

	err := rows.Scan(&i.ID, &i.SellerID)
...
func GetItemsBeingSoldByUser(userID string, conn *pgx.Conn) ([]Item, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			fmt.Printf("Error scannig items %v", err)
			continue
		}
...
func FindItemById(id uuid.UUID, conn *pgx.Conn) (Item, error) {
	rows := conn.QueryRow(context.Background(), "SELECT title, notes, seller_id, price_in_cents FROM item WHERE id = $1", id)
	item := Item{
		ID: id,
	}
	err := rows.Scan(&item.Title, &item.Notes, &item.SellerID, &item.PriceInCents)
	if err != nil {
